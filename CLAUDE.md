...
| **Tools (15 .py + MCP)** | [tools/](tools/) | 100% non-LLM | `state, blockers, progress, decisions, cycle, tasks, stamp, objective, priorities, questions, group, run-tests` (12 functional modules) + `mcp_server.py` + `_paths.py` + `__init__.py`. `run-tests` unified regression runner (**323/323 aggregate across 17 test files** — 12 hook tests under `.claude/hooks/tests/` + 5 tool tests under `tools/tests/` (test-group.py + test-priorities.py + test-objective.py + test-decisions-tier3.py + test-state.py); empirically verified 2026-07-03 via `HOME=<repo> python3 -m tools.run-tests` on the current `main` tree. NOTE per Hard Rule 15: an earlier "322/322 across 14 files" figure appeared in the docs but was never in committed history — `git ls-tree` shows this tree only ever carried 11-12 test files; the current empirical count is authoritative). `group` is chain/group/tree composition primitive (Q1 Layer A, canonical taxonomy from second-brain). Per-tool index + composition map at [tools/README.md](tools/README.md) (DRAFT v1 — 2026-05-06 evening). |
