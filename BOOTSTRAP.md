...
| **Hooks** (13 wired across 8 events) | `.claude/hooks/*.{sh,py}` | ~85% (`additionalContext` JSON) | PreToolUse (3: policy-block, malware-block, opt-write-block) + PostToolUse (1: leak-detector) + SessionStart (2: session-start, session-orient) + UserPromptSubmit (3: context-warning, output-discipline-guard agent-discipline-gate per SB-108, mode-enforcement per SB-056) + PreCompact (1: writes handoff doc) + PostCompact (1: directs /orient + references handoff) + Stop (1: end-of-cycle-stamp per SB-114/115 DRAFT) + SessionEnd (1: summary). Regression tests at `.claude/hooks/tests/*.py`. |
...
| **Tools** (10 + MCP server) | `tools/*.py` | 100% deterministic non-LLM | `state.py`, `blockers.py`, `progress.py`, `decisions.py`, `cycle.py`, `tasks.py`, `stamp.py` (SB-115), `objective.py` (SB-118) — invoked by commands; also exposed via `tools/mcp_server.py` (FastMCP, 6 tools) for MCP-aware consumers |
