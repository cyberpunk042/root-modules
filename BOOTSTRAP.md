...
- [hook-architecture.md](.claude/rules/hook-architecture.md) — 2-layer hook design + the 13 wired machine-level hook fires across 8 events
...
|---|---|---|---|
| **Slash commands** (28) | `.claude/commands/*.md` | 100% on invoke | Operator-driven workflows. `/orient`, `/cycle`, `/mode-{pm,architect,dual,status,clear}`, `/blockers`, `/progress`, `/decisions`, `/log`, `/audit`, `/sync-progress`, `/help-root`, `/handoff`, `/stamp-{horizontal,vertical,on,off,auto,status}` (SB-115 stamp config), `/install-agent-brain`, `/mission`, `/focus`, `/impediment` (SB-118 objective layer), `/priorities` (SB-127 imminent-work hot-queue), `/terminate`, `/finish-smoothly` (operator-authored session-termination prep) |
...
| **Hooks** (14 wired across 8 events) | `.claude/hooks/*.{sh,py}` | ~85% (`additionalContext` JSON) | PreToolUse (3: policy-block, malware-block, opt-write-block) + PostToolUse (1: leak-detector) + SessionStart (2: session-start, session-orient) + UserPromptSubmit (4: context-warning, output-discipline-guard agent-discipline-gate per SB-108, mode-enforcement per SB-056, mindfulness per SB-126) + PreCompact (1: writes handoff doc) + PostCompact (1: directs /orient + references handoff) + Stop (1: end-of-cycle-stamp per SB-114/115 DRAFT) + SessionEnd (1: summary). Regression tests at `.claude/hooks/tests/*.py` (6 test files / 72 cases / all green). |
...
| **Tools** (11 + MCP server) | `tools/*.py` | 100% deterministic non-LLM | `state.py`, `blockers.py`, `progress.py`, `decisions.py`, `cycle.py`, `tasks.py`, `stamp.py` (SB-115), `objective.py` (SB-118), `priorities.py` (SB-127) — invoked by commands; also exposed via `tools/mcp_server.py` (FastMCP, 7 tools incl. root_objective) for MCP-aware consumers |
...
| **Rules files** (11) | `.claude/rules/*.md` | Load-on-demand | Topic-specific guidance. Includes `trigger-model.md` (unified signal→action→recovery model across all 8 mechanism types — see DESIGN.md) + `compound-and-waterfall.md` (SB-123: two-axes design pattern — compound = additive layering at-a-moment; waterfall = sequential cascade event-to-event). |
