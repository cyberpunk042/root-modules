...
After cooling-off period (T057), operator decides whether to flip `auto_connect` for root-ghostproxy in `/opt/devops-solutions-information-hub/wiki/config/sister-projects.yaml` from `false` to `true`.

## Trade-off

| Keep `false` (status quo) | Flip to `true` |
|---|---|
| Friction-by-design: type=root projects gate security envelope; explicit-authorization gate via --connect-project preserved | Convenience: subsequent `tools.setup` runs auto-hookup root-ghostproxy when path resolves |
